...
try {
  ...
  if (!$user->_logged_in) {
   ...
   case 'watch':
    ...
    /* ================= GET POSTS ================= */
    $videos_discover = ($selected_country)
        ? $user->get_posts(['get' => 'discover', 'filter' => 'video', 'country' => $selected_country['country_id']])
        : $user->get_posts(['get' => 'discover', 'filter' => 'video']);

    $videos_newsfeed = ($selected_country)
        ? $user->get_posts(['get' => 'newsfeed', 'filter' => 'video', 'country' => $selected_country['country_id']])
        : $user->get_posts(['get' => 'newsfeed', 'filter' => 'video']);

    $reels_discover = ($user->_logged_in)
        ? $user->get_posts(['get' => 'discover', 'filter' => 'reel'])
        : [];
    ...
    foreach ($all_posts as $post) {
        if (!in_array($post['post_id'], $seen_ids)) {
            $seen_ids[] = $post['post_id'];
            $unique_posts[] = $post;
        }
    }
    ...
    /* ================= ASSIGN TO SMARTY ================= */
    $smarty->assign('posts', $unique_posts);
...
case 'reel':
    ...
    if ($user->_logged_in) {
        $all_reels = array_merge($all_reels, $user->get_posts([
            'get'    => 'discover',
            'filter' => 'reel'
        ]));
    }
    ...
    foreach ($all_reels as $reel) {
        ...
        // 🔥 CANONICAL POST ID (MOST IMPORTANT)
        $canonical_id = (int)(
            $reel['origin_post_id']
            ?? $reel['shared_post_id']
            ?? $reel['post_id']
        ...
        // Skip duplicates (REAL FIX)
        if (isset($seen_ids[$canonical_id])) {
            continue;
        }
...
if ($user->_logged_in && $system['packages_enabled']) {

    $user_id = (int) $user->_data['user_id'];

    $get_data = $db->query("
        SELECT 
            users.user_subscription_date,
            users.user_subscribed,
            packages.period,
            packages.period_num
        FROM users
        LEFT JOIN packages 
            ON users.user_package = packages.package_id
    ...
    if ($get_data && $get_data->num_rows > 0) {
        ...
        if ($data['user_subscribed']) {
            ...
            switch ($data['period']) {
                case 'day':   $duration = 86400; break;
                case 'week':  $duration = 604800; break;
                case 'month': $duration = 2629743; break;
                case 'year':  $duration = 31556926; break;
                default:      $duration = 0;
            }

            if ($duration > 0) {

                $subscription_end = strtotime($data['user_subscription_date']) 
                                  + ($data['period_num'] * $duration);

                $time_left = $subscription_end - time();
                ...
                if ($time_left > 0) {
                    ...
                    /* ⏰ URGENT: 20 HOURS */
                    if ($time_left <= (20 * 60 * 60)) {

                        $subscription_alert = sprintf(
                            __("️ Your subscription will expire in <b> %d hours %d minutes </b>. Please renew immediately to avoid service interruption."),
                            max(0, $hours),
                            max(0, $minutes)
                        );

                    }
                    /*  EARLY WARNING: 6 DAYS */
                    elseif ($time_left <= (6 * 24 * 60 * 60)) {

                        $subscription_alert = sprintf(
                            __("ℹ️ Your subscription will expire in <b> %d days %d hours </b>. Renew now to continue uninterrupted access."),
                            max(0, $days),
                            max(0, $hours)
                        );
                    }
...
/* assign to smarty */
$smarty->assign('subscription_alert', $subscription_alert);
  // get ads
  $ads = $user->ads('home');
  /* assign variables */
