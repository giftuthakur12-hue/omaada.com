 ...
 * @author Zamblek
 */
//  var_dump($user->_data['user_group']);
//  exit;
...
// if ($user->_is_moderator && isset($moderator_mode) && !$moderator_mode) {
//   _error(__('System Message'), __("You don't have the right permission to access this"));
// }
...
case '':
  ...
  $cache_file = __DIR__ . '/cache_dashboard.json';
  $cache_time = 60; // seconds (1 min)
  ...
  if (file_exists($cache_file) && (time() - filemtime($cache_file)) < $cache_time) {
    ...
    $users_result = $db->query("
      SELECT 
        COUNT(*) as users,
        SUM(user_approved = 0) as pending,
        SUM(user_activated = 0) as not_activated,
        SUM(user_banned = 1) as banned
      FROM users
    ");
    ...
    $insights = [
      ...
      'pending' => $users_data['pending'],
      'not_activated' => $users_data['not_activated'],
      'banned' => $users_data['banned'],
      'posts' => $db->query("SELECT COUNT(*) as count FROM posts")->fetch_assoc()['count'],
      'comments' => $db->query("SELECT COUNT(*) as count FROM posts_comments")->fetch_assoc()['count'],
      'pages' => $db->query("SELECT COUNT(*) as count FROM pages")->fetch_assoc()['count'],
      'groups' => $db->query("SELECT COUNT(*) as count FROM `groups`")->fetch_assoc()['count'],
      'events' => $db->query("SELECT COUNT(*) as count FROM events")->fetch_assoc()['count'],
      'messages' => $db->query("SELECT COUNT(*) as count FROM conversations_messages")->fetch_assoc()['count'],
      'notifications' => $db->query("SELECT COUNT(*) as count FROM notifications")->fetch_assoc()['count']
    ];

    //  FAST CHART
    $chart = [];
    $chart_result = $db->query("
      SELECT 
    ...
    while ($row = $chart_result->fetch_assoc()) {
      $chart['users'][$row['month']] = $row['total'];
    }
